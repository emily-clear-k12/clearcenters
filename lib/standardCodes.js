...
import { QUESTS as EXPEDITION_QUESTS } from "./cases/expedition-station/catalog.js";
...
export function frUnitKey(standard) {
  const raw = String(standard || "").trim();
  if (!raw) return "";
  if (FR_UNITS[raw]) return raw;
  const compact = raw.toUpperCase().replace(/\./g, "-").replace(/-BONUS$/i, "");
  const keys = Object.keys(FR_UNITS).sort((a, b) => b.length - a.length);
  for (const key of keys) {
    const keyCompact = key.toUpperCase().replace(/\./g, "-");
    if (compact === keyCompact || compact.startsWith(`${keyCompact}-`)) return key;
  }
  return "";
}

export function frTopicName(standard) {
  const key = frUnitKey(standard);
  if (!key) return "";
  return key.replace(/^(?:SS\.)?\d+\.FR\./i, "").replace(/-/g, " ");
}

export function codesFor(standard) {
  const raw = String(standard || "").trim();
  const unit = frUnitKey(raw);
  if (unit) return FR_UNITS[unit];
  // Expedition Station quests: main standard first, then supporting standards,
  // so a quest shows up under each of them on the Assign page.
  const quest = EXPEDITION_QUESTS && EXPEDITION_QUESTS[raw];
  if (quest && quest.teks) return uniqueCodes([quest.teks, ...(quest.supportingTeks || [])].join(" "));
...
export function coverLine(standard, learningTarget) {
  const target = String(learningTarget || "").replace(/\s+/g, " ").trim();
  if (target) return target;
  const label = missionMapTeksLabel(standard) || "";
