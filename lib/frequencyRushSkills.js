...
// Sept 24, 2026 — AUTHORED sets (ELAR word study, §11.5): 30 written items
// each in lib/cases/frequency-rush/skills/, drafted by Claude and reviewed by
// Emily. Every item keeps its own `answer`; the site shuffles the choices.
// Checked by tools/frequency-rush-skillcheck.cjs.
for (const [standard, bank] of Object.entries(SKILL_BANKS)) {
  SKILL_SETS[standard] = { title: bank.title, kind: "authored", bank, labels: bank.labels };
}
...
// Stable answer-button id for a choice, so the id doesn't give away which
// choice is right (a fixed "first choice wins" id would).
...
function authoredItem(bank, item) {
  const bins = shuffle(item.choices).map((text) => ({ id: choiceId(text), label: text }));
  return {
    type: "sort_bins",
    id: item.id,
    prompt: item.prompt,
    bins,
    correctBinId: choiceId(item.answer),
    shuffleBins: false, // already shuffled here
    explanation: item.explanation,
    bankId: `skill:${bank.standard}`,
    metadata: { skill: bank.standard },
  };
}
...
function buildItem(op, a, b) {
  ...
  return {
    ...
    shuffleBins: false, // already shuffled here
    // Shown after an answer. Without it the game prints "7 × 8 = ?: ".
    explanation: def.prompt(a, b).replace("?", String(ans)) + ".",
    bankId: `skill:${op}`,
    metadata: { skill: op },
  };
}
...
export function buildSkillItems(standard, poolSize = SKILL_POOL_SIZE) {
  ...
  if (!set) return [];
  // Authored sets hand the game every item; it draws each run's rounds from them.
  if (set.kind === "authored") return shuffle(set.bank.items.map((item) => authoredItem(set.bank, item)));
  const items = [];
  const seen = new Set();
  let guard = 0;
...
export function recomputeSkillItem(standard, itemId) {
  ...
  if (set.kind === "authored") {
    const item = set.bank.items.find((it) => it.id === id);
    return item ? { id, correctBinId: choiceId(item.answer), prompt: item.prompt } : null;
  }
...
// Sept 24, 2026 — My Missed Words: rebuilds the full question for a saved
// item key ("mul:7x8", "ela4hom:05"), with fresh shuffled choices, so it can
// come back in the next run. Returns null if the key isn't part of this set.
export function skillItemForKey(standard, key) {
  const set = getSkillSet(standard);
  if (!set) return null;
  if (set.kind === "authored") {
    const item = set.bank.items.find((it) => it.id === String(key));
    return item ? authoredItem(set.bank, item) : null;
  }
  const parsed = recomputeSkillItem(standard, key);
  return parsed ? buildItem(parsed.op, parsed.a, parsed.b) : null;
}
