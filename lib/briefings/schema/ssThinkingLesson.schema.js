...
export function auditThinkingQuality(lesson, opts) {
  ...
  if (rs && Array.isArray(rs.items) && Array.isArray(rs.bins)) {
    ...
    rs.items.forEach((item) => {
      ...
      String(item.text || "")
        ...
        .filter((w) => w.length > 3 && !STOP.has(w))
        .forEach((w) => {
          if (!original[stem(w)]) original[stem(w)] = w;
        });
  ...
  const FEEDBACK_KEYS = /^(why|whatIf|response|nameWrong|rightMessage|wrongMessage|hint|bestLead|helpWrong|helpPass|whoseRight|whoseWrong|distractorFailMessage)$/;
  const SKIP_KEYS = /^(id|type|mode|icon|emoji|color|imageKey|reasonId|sceneId|isTrait|ok|best|x|y)$/;
