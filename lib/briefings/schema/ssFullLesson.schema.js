...
export function validateSsFullLessonShape(lesson) {
  ...
  const expectedTypes = SS_FULL_LESSON_PHASES.map((p) => p.type);
  const actualTypes = Array.isArray(lesson.phases) ? lesson.phases : [];
  if (JSON.stringify(actualTypes) !== JSON.stringify(expectedTypes)) {
    errors.push(
      `phases must be exactly ${JSON.stringify(expectedTypes)} for the SS full-lesson shape (any content variation belongs INSIDE a phase, not in a different phase list) — got ${JSON.stringify(actualTypes)}`
    );
  }
  ...
  if (lesson.practice) {
    errors.push(...validateMechanicPublicData(lesson.practice.mechanic, lesson.practice.data));
  } else {
    errors.push('missing "practice" phase content');
  }
