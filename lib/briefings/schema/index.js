...
// Type 3 — people. See ssPeopleLesson.schema.js for why its decision step has
// no right answer (real people made real mistakes; one of 3.1B's three was
// fired for the choice he made).
export {
  SS_PEOPLE_PHASES,
  SS_PEOPLE_TARGET_MINUTES,
  SS_PEOPLE_MINUTES_RANGE,
  SS_PEOPLE_CLEARANCE_ITEMS,
  validateSsPeopleShape,
  auditPeopleQuality,
} from "./ssPeopleLesson.schema";

// Type 4 — categories. See ssCategoryLesson.schema.js for why the graded step
// names the RULE rather than the category (naming the category is the sort
// again with extra steps).
export {
  SS_CATEGORY_PHASES,
  SS_CATEGORY_TARGET_MINUTES,
  SS_CATEGORY_MINUTES_RANGE,
  SS_CATEGORY_CLEARANCE_ITEMS,
  SS_CATEGORY_MIN_BOUNDARIES,
  validateSsCategoryShape,
  auditCategoryQuality,
} from "./ssCategoryLesson.schema";
...
import { validateSsPeopleShape } from "./ssPeopleLesson.schema";
import { validateSsCategoryShape } from "./ssCategoryLesson.schema";
...
export function validateLessonShape(lesson) {
  ...
  if (lesson.shape === "ssPeople") return validateSsPeopleShape(lesson);
  if (lesson.shape === "ssCategory") return validateSsCategoryShape(lesson);
