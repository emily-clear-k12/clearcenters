...
export function crossCheckLesson(pub, srv) {
  ...
  // --- the locked prediction, for every shape that settles one --------------
  // openingFrame locks a guess and the teach phase pays it off. Type 1 does it
  // through `firstReasonId`; the other three name the winning option directly
  // in `resolvesPredictionTo`. Either way the id has to be one of the options
  // the student was actually offered, or a student who guessed right is told
  // they guessed wrong — which is worse than not asking.
  ["sideBySide", "theDecision", "wrongDesk"].forEach((phaseKey) => {
    if (!has(phaseKey) || !has("openingFrame")) return;
    const resolves = (pub[phaseKey] || {}).resolvesPredictionTo;
    const predictIds = ids((pub.openingFrame || {}).predictOptions);
    if (!resolves) {
      p(`${phaseKey} has no resolvesPredictionTo — the locked opening prediction can never be paid off as right`);
      return;
    }
    if (!predictIds.includes(resolves)) {
      p(`${phaseKey} resolves the opening prediction to "${resolves}", which is not one of openingFrame.predictOptions`);
    }
  });
