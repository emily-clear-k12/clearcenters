...
/** Same key as usePlanner / demoStudentDay — extras only. */
const ADDED_ACTIVITIES_KEY = "ci2.teacher.addedActivities";

/** Session flag so Day → Week navigation can re-show Apply toast with Undo. */
export const SUNDAY_PENDING_UNDO_KEY = "ci2.sunday.pendingUndo";

export function removeSundayBridgeActivities() {
  if (typeof window === "undefined") return { removed: 0, ids: [] };
  let removed = 0;
  const ids = [];
  try {
...
export function writeSundayPendingUndo(meta) {
  if (typeof window === "undefined") return;
  try {
    window.sessionStorage.setItem(
      SUNDAY_PENDING_UNDO_KEY,
      JSON.stringify(meta || {})
    );
  } catch {
    /* ignore */
...
export function readSundayPendingUndo() {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.sessionStorage.getItem(SUNDAY_PENDING_UNDO_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    return parsed && typeof parsed === "object" ? parsed : null;
  } catch {
...
export function clearSundayPendingUndo() {
  if (typeof window === "undefined") return;
  try {
    window.sessionStorage.removeItem(SUNDAY_PENDING_UNDO_KEY);
  } catch {
    /* ignore */
  }
}
