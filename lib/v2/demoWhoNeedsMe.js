// CI2.0 Check-ins (UI) — reteach / small-group stub. User path: /v2/teacher/check-ins
...
/** @deprecated Use CHECK_INS_HREF — same value; kept so callers keep working. */
export const WHO_NEEDS_ME_HREF = CHECK_INS_HREF;
/** Legacy URL — page redirects to CHECK_INS_HREF. */
...
/** Shared with Daily Focus / This Week / TeacherSubnav — selected period room. */
 ...
 * "all" → first class in current setup (RoomCards never leave an unscoped lens).
 */
...
/** Demo "today" day index — matches Daily Focus default d=2 (Wed). */
export const FOCUS_TODAY_DAY = 2;

/** Demo "tomorrow" day index — Reteach tomorrow lands here (Thu). */
...
export const DEMO_WHO_NEEDS_ME = [
  // Softest Reports cluster (5.6B) — same who as class story, period A default.
  {
    id: "wnm-sofia",
    studentFirst: "Sofia",
    subject: "science",
    ...
    tone: "needs_you",
    blurb: "Needs a look — claim without an evidence note; mixture/solution still swapped.",
    nextHint: "Small group with Noah · Diego: claim + one evidence note.",
    source: "demo",
    periodId: "A",
  },
  {
    ...
    reason: "low_clearance",
    tone: "needs_you",
    blurb: "Needs a look — wrote a sort, skipped the evidence sentence.",
    nextHint: "Pull with Sofia · Diego on claim + one evidence note.",
    source: "demo",
    periodId: "A",
  },
  {
    ...
    reason: "low_clearance",
    tone: "needs_you",
    blurb: "Needs a look — terms mixed; evidence line empty.",
    nextHint: "Same soft cluster as Reports — short Check-in, then Daily Focus reteach.",
    source: "demo",
    periodId: "A",
  },
  // Secondary Reports soft (4.3D) — other periods; same cast language.
  {
    id: "wnm-kai",
    studentFirst: "Kai",
    subject: "math",
    ...
    reason: "low_clearance",
    tone: "needs_you",
    blurb: "Mixed — reasons thin when models don’t match.",
  ...
  },
  {
    ...
    reason: "not_started",
    tone: "needs_you",
    blurb: "Hasn’t started today’s compare task yet.",
 ...
 *   classFilter — TEACHER_SETUPS class key (A/B/C) or "all"; filters kid cards.
 *   inheritPeriodId — stamp live rows without period onto this room (defaults to classFilter).
 *   limit — max cards (default WHO_NEEDS_ME_MAX); null = no cap (for needsByClass counts).
 */
...
export function getWhoNeedsCountsByClass(
  ...
  for (const c of cards) {
    ...
    if (pid && map[pid] != null) map[pid] += 1;
      ...
      // Untagged → count toward every room lightly? Prefer first key only.
...
function focusBlockTitle(kind, subject, assignment) {
  if (kind === "reteach") {
    return assignment ? `Reteach · ${assignment}` : "Reteach tomorrow (stub)";
  }
  return assignment ? `Small group · ${assignment}` : "Small group pull";
}

/**
 * Pull a Check-ins card onto Daily Focus: small group → today; reteach → tomorrow.
...
export function pushCheckInToDailyFocus(card, action, opts = {}) {
  ...
  // Small group → today; Reteach tomorrow → tomorrow (unless caller overrides day).
...
export function whoNeedsGlanceText(count) {
  ...
  if (count === 1) return "1 kid for a quick check-in — ready when you are.";
  return `${count} kids for a quick check-in — ready when you are.`;
}
 ...
 * Reuses Check-ins list, focusBlocks, and whoNeedsMe.choices — no new storage.
...
export function getKidCheckInMotion(studentFirst, confirmedIds = loadConfirmedIds()) {
  ...
  if (openCard) {
    ...
    return {
      ...
      label: needsYou ? "On Check-ins · needs you" : "On Check-ins · ready",
      samLine: openCard.blurb
        ? openCard.blurb
        : needsYou
          ? `${openCard.studentFirst} is waiting on Check-ins.`
          : `${openCard.studentFirst} is on Check-ins — calm look when ready.`,
      cardId: openCard.id,
  ...
  if (blocks.length) {
    ...
    return {
      ...
      label: isReteach ? "Reteach · in motion" : "Small group · in motion",
      samLine: isReteach
        ? `Reteach is lined up — ${display} is on the tomorrow block.`
 ...
 * Reuses pushCheckInToDailyFocus — synthesizes cards from DEMO_WHO_NEEDS_ME (or opts.names).
...
export function pushStandardClusterToToday(standardCode, opts = {}) {
  ...
  // Stamp 8-min reteach title onto the merged block (same store — no second system).
  ...
  if (idx >= 0) {
    ...
    const who = names.length ? names.join(" · ") : "soft cluster";
    all[idx] = {
      ...all[idx],
      minutes: 8,
      title: `Reteach · ${code} · 8 min`,
      standard: code,
      at: new Date().toISOString(),
    };
    saveFocusBlocks(all);
...
export function pushCheckInToThisWeek(card, opts = {}) {
  ...
  const entry = {
    ...
    day,
    kind: "work",
    title: `Check-ins · ${card.studentFirst} · ${card.assignment || "small group"}`,
