...
/** Ids SAM already celebrated for Teacher checked (once per stamp, same browser). */
...
export function loadCelebratedTeacherCheckedIds() {
  if (typeof window === "undefined") return [];
  try {
    const raw = window.localStorage.getItem(TEACHER_CHECKED_CELEBRATED_KEY);
    const parsed = raw ? JSON.parse(raw) : [];
    return Array.isArray(parsed) ? parsed.filter((x) => typeof x === "string") : [];
  } catch {
    return [];
...
export function markTeacherCheckedCelebrated(ids) {
  if (typeof window === "undefined" || !ids?.length) return;
  try {
    const prev = loadCelebratedTeacherCheckedIds();
    const next = [...prev];
    for (const id of ids) {
