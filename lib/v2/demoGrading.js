...
/**
 * Map grading studentFirst → demo kid id (Sofia/Noah/Diego/Leo/Kai/Riley).
...
export function resolveMissionIdsForStamp(item) {
  ...
  if (/(mixture|mixtures|solution)/.test(title) && !ids.includes("stu-soft-mixtures")) {
    ids.push("stu-soft-mixtures");
  }
  const std = String(item.standard || "").trim().toUpperCase();
  if (std === "5.6B" && !ids.includes("stu-soft-mixtures")) ids.push("stu-soft-mixtures");
  return ids;
}

/** Mark mission/practice cards on student My Day as teacher-checked (calm stamp).
 * Writes into the demo kid namespace matching studentFirst (soft cast + Leo/Kai/Riley). */
...
/** Pitch cast — show these first in Grading so inbox matches the class story. */
const PITCH_CAST_NAMES = new Set(["Sofia", "Noah", "Diego"]);
function pitchCastRank(s) {
  return PITCH_CAST_NAMES.has(String(s.studentFirst || "").trim()) ? 0 : 1;
}
...
export function getPendingSubmissions(confirmedIds = loadConfirmedIds(), sortMode = "oldest") {
  ...
  const demo = sortSubmissions(
    ...
    sortMode
  ).slice().sort((a, b) => pitchCastRank(a) - pitchCastRank(b) || 0);
  return [...live, ...demo];
}
