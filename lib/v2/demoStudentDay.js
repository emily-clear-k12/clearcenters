// CI2.0 Student My Day — soft cast Sofia · Noah · Diego + Leo / Kai / Riley.
// Missions mirror teacher demo week "today" = Wednesday (day index 2).
// Soft cast leads with TEKS 5.6B mixtures so teacher loop lands on the right kid.
...
/** Active demo kid id — soft cast first (Sofia · Noah · Diego), then Leo / Kai / Riley. */
...
export function setActiveDemoKidId(id) {
  ...
  const next = DEMO_STUDENT_KIDS.some((k) => k.id === resolved) ? resolved : DEFAULT_DEMO_KID_ID;
  try {
