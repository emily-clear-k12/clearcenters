...
/** Active demo kid id — Leo / Kai / Riley switcher on My Day. */
export const DEMO_KID_STORAGE_KEY = "ci2.student.demoKid";

/** Light demo roster — not a full class list. */
...
/** @deprecated Prefer getActiveDemoStudent() — kept as Leo default for older imports. */
...
export function getActiveDemoKidId() {
  ...
  try {
    ...
    if (raw && DEMO_STUDENT_KIDS.some((k) => k.id === raw)) return raw;
  } catch {
    /* ignore */
  }
  return "leo";
...
export function setActiveDemoKidId(id) {
  ...
  const next = DEMO_STUDENT_KIDS.some((k) => k.id === id) ? id : "leo";
  try {
...
/**
 * Per-kid localStorage key so Leo / Kai / Riley demos don't collide.
...
export function readStudentScopedRaw(baseKey) {
  if (typeof window === "undefined") return null;
  try {
    const scoped = window.localStorage.getItem(studentScopedKey(baseKey));
    if (scoped != null) return scoped;
    if (getActiveDemoKidId() === "leo") {
...
export function writeStudentScopedRaw(baseKey, value) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem(studentScopedKey(baseKey), value);
  } catch {
    /* ignore quota */
  }
}
