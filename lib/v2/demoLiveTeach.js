 ...
 * Stamps ci2.teacher.classFilter (same pattern as Library Add) so departmentalized rooms see the tile.
 */
export function mintLiveTeachPracticeToPlanner(shellOrAct, opts = {}) {
...
export function assignPracticeToMyDay(shellOrAct, opts = {}) {
  if (typeof window === "undefined" || !shellOrAct?.id) return null;
  const classKey = opts.classKey != null ? opts.classKey : null;
  const classes = resolveLibraryClasses(classKey);
  const periodId =
    Array.isArray(classes) && classes[0] ? classes[0] : null;
  const entry = {
    id: shellOrAct.id,
  ...
  mintLiveTeachPracticeToPlanner(shellOrAct, { classKey });
  return entry;
}
