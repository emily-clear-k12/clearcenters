...
/** localStorage — teacher "Assign practice" stubs (same browser as student My Day). */
...
export function loadAssignedPractice() {
  ...
  try {
    const raw = localStorage.getItem(PRACTICE_ASSIGNED_KEY);
    if (!raw) return [];
...
export function assignPracticeToMyDay(shellOrAct, opts = {}) {
  ...
  try {
    localStorage.setItem(PRACTICE_ASSIGNED_KEY, JSON.stringify(next));
