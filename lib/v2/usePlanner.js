...
import {
  ...
  removeSundayBridgeActivities,
  readSundayPendingUndo,
  clearSundayPendingUndo,
} from "./demoSundayBridge";

const LS_SETUP = "ci2.teacher.setupKey";
const LS_CLASS = "ci2.teacher.classFilter";
/** JSON map setupKey → last period/room key (departmentalized restore). */
const LS_CLASS_BY_SETUP = "ci2.teacher.classFilterBySetup";
...
/** Session-only: soft restore toast once so Day↔Week remounts stay quiet. */
const SS_ROOM_RESTORE_TOAST = "ci2.setup.roomRestoredToast";
...
function readClassBySetup() {
  const raw = readLS(LS_CLASS_BY_SETUP);
  if (!raw) return {};
  try {
...
function writeClassBySetup(setupKey, classKey) {
  if (!setupKey || !classKey) return;
  const next = { ...readClassBySetup(), [setupKey]: classKey };
  writeLS(LS_CLASS_BY_SETUP, JSON.stringify(next));
}

function roomLabelFor(setupKey, classKey) {
  const setup = TEACHER_SETUPS[setupKey] || TEACHER_SETUPS.self;
  const cls = setup.classes.find((c) => c.key === classKey) || setup.classes[0];
  const room = cls?.name || classKey || "class";
  const short = (setup.label || "").split("·")[0].trim() || "Setup";
  return `${room} · ${short}`;
}

function notifyClassFilterUpdated() {
  if (typeof window === "undefined") return;
  try {
    window.dispatchEvent(new CustomEvent("ci2-class-filter-updated"));
  } catch {
    /* ignore */
  }
}
...
export function usePlanner() {
  ...
  useEffect(() => {
    ...
    const bySetup = readClassBySetup();
    const key = sk && TEACHER_SETUPS[sk] ? sk : "self";
    const setup = TEACHER_SETUPS[key] || TEACHER_SETUPS.self;
    const remembered = bySetup[key] || readLS(LS_CLASS);
    const ck =
      remembered && setup.classes.some((c) => c.key === remembered)
        ? remembered
        : setup.classes[0]?.key || "A";
    const lv = parseLevels(readLS(LS_LEVELS));
    const subjects = setup.subjects;
    if (sk && TEACHER_SETUPS[sk]) setSetupKeyState(sk);
    setClassFilterState(ck);
    writeLS(LS_CLASS, ck);
    writeClassBySetup(key, ck);
    ...
    const sundayMeta = readSundayAppliedMeta();
    if (sundayMeta && (sundayMeta.count > 0 || (Array.isArray(sundayMeta.ids) && sundayMeta.ids.length))) {
      setSundayApplied(true);
    }
    // Soft toast once per session when a non-default setup/room comes back (or none).
    try {
      const firstKey = setup.classes[0]?.key || "A";
      const noteworthy = key !== "self" || (setup.classes.length > 1 && ck !== firstKey);
      const hadStored = !!(sk && TEACHER_SETUPS[sk]) || !!bySetup[key] || !!readLS(LS_CLASS);
      const already = sessionStorage.getItem(SS_ROOM_RESTORE_TOAST);
      if (hadStored && noteworthy && !already) {
        sessionStorage.setItem(SS_ROOM_RESTORE_TOAST, "1");
        setToast({ text: `Back in ${roomLabelFor(key, ck)}.` });
      }
    } catch {
      /* private mode / no sessionStorage — skip toast */
    }
    setHydrated(true);
  }, []);

  // Cross-tab / external: rehydrate dial so Sunday preview blocks stay live with Hands-off.
  useEffect(() => {
    if (!hydrated) return;
    const pull = () => {
      const lv = parseLevels(readLS(LS_LEVELS));
    ...
    const onStorage = (e) => {
      if (e.key === LS_LEVELS) pull();
    };
    window.addEventListener("storage", onStorage);
    window.addEventListener("ci2-hands-off-updated", pull);
    return () => {
      window.removeEventListener("storage", onStorage);
      window.removeEventListener("ci2-hands-off-updated", pull);
    };
  }, [hydrated]);
  ...
  function setSetupKey(key) {
    ...
    const setup = TEACHER_SETUPS[key];
    const remembered = readClassBySetup()[key];
    const nextClass =
      remembered && setup.classes.some((c) => c.key === remembered)
        ? remembered
        : setup.classes[0]?.key || "A";
    setClassFilterState(nextClass);
    writeLS(LS_CLASS, nextClass);
    writeClassBySetup(key, nextClass);
    notifyClassFilterUpdated();
  ...
  function setClassFilter(key) {
    ...
    writeClassBySetup(setupKey, key);
    notifyClassFilterUpdated();
  }
  ...
  function setLevel(key, opts = {}) {
    ...
    // After persist — Sunday preview copy/blocks re-derive from levels (same tab + listeners).
    if (typeof window !== "undefined" && nextLevels) {
      window.dispatchEvent(
        new CustomEvent("ci2-hands-off-updated", { detail: { levels: nextLevels, key, targets } })
      );
    }
  ...
  useEffect(() => {
    ...
    if (!ok) {
      ...
      writeClassBySetup(setupKey, first);
      notifyClassFilterUpdated();
    }
  ...
  // Re-show Apply toast with Undo after Day → Week navigation (Library/Check-ins pattern).
  useEffect(() => {
    if (!hydrated) return;
    const pending = readSundayPendingUndo();
    if (!pending || !pending.count) return;
    clearSundayPendingUndo();
    const count = Number(pending.count) || 0;
    setSundayApplied(true);
    setToast({
      text: `Applied ${count} block${count === 1 ? "" : "s"} to This Week.`,
      undo: () => {
        removeSundayBridgeActivities();
        setActivities((prev) => {
        ...
        setToast({ text: "Undone — Sunday tiles removed from This Week." });
      },
    });
  }, [hydrated]);
  ...
  /**
   * Undo Sunday → This Week: remove minted Sunday tiles (same pattern as Library / Check-ins).
   * Survives Day → Week navigation via session pending-undo + applied meta.
   */
  function undoSundayToThisWeek() {
    removeSundayBridgeActivities();
    setActivities((prev) => {
      const next = prev.filter((a) => !isSundayBridgeActivity(a));
      persistAdded(next);
