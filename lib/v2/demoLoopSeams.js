...
export function buildFocusWatchSeam({ subject, className } = {}) {
  ...
  if (urgency === "advanced" && landedCode) {
    ...
    if (landedKind === "evidence") {
      watch = landedWho
        ? `${landedWho} look clearer on ${skill} · you're covered. Next: keep Daily Focus pace.`
        : `${skill} looks clearer · you're covered. Next: keep Daily Focus pace.`;
    } else {
      watch = landedWho
...
export function buildFocusWinSeam({ subject, className, fallbackWin } = {}) {
  ...
  // Quiet win when softest story already landed (reteach / sit / evidence).
