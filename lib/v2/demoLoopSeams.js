...
export function focusBlockProvenanceLine(block) {
  if (block && (block.fromReports || block.source === "reports")) {
    ...
    return "from Reports · same kids still on your radar";
  }
...
export function getCheckInsStoryBand() {
  ...
  const allClear = urgency === "cooled";
  return {
    title: allClear ? "Reports · clear" : "Reports · needs a look",
...
export function getAllClearSurfaceLine(surface = "loop") {
  ...
  if (surface === "reports") {
    return "Reports look clear for now — soft urgency cooled. Soft cluster still findable if you drill in.";
  }
...
export function getGradingStoryBand(opts = {}) {
  ...
  return {
    title: resolved ? "Reports · covered" : "Reports",
    line,
