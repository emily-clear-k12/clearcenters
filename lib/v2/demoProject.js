...
/** Student Later · Project Open shell. */
export const STUDENT_PROJECT_HREF = (id) =>
  `/v2/student/project/${encodeURIComponent(id)}`;

const DAY_FOCUS_HREF = (day) => `/v2/teacher/day?d=${Number(day) || 2}`;

/** Who it's for — whole class vs small group (ties to Check-ins / reteach). */
export const AUDIENCE_OPTIONS = [
  ...
  },
  {
    ...
    label: "Small group",
    short: "Small group",
    hint: "Pull a few kids — often from Check-ins.",
  },
];
...
/** Materials + time stub for one-pager right column (not a real kit list). */
export function materialsStub(act, span) {
  const mins = act?.minutes || 15;
  const days = Math.max(1, Number(span?.days) || 1);
  const bySub = {
    math: "Fraction strips or number line · exit postcard paper",
    elar: "Passage / sticky notes · short-response sheet",
    science: "Lab tray or observation sheet · poster half-sheet",
    social: "Map or region card · postcard paper",
  };
  const kit = bySub[act?.subject] || "Paper + pencil · calm work space";
  const time =
    days === 1
      ? `~${mins} min`
      : `~${mins} min/day · ${days} days`;
  return `${kit} · ${time}`;
}

/** Ultra-short standard one-liner for the right column (not a picker). */
export function standardOneLiner(act) {
  const code = act?.standard;
  if (!code) return "Add TEKS when ready — teach heart first.";
  const bySub = {
    math: "Kids show equivalent ideas with a clear model.",
    elar: "Kids name the author’s purpose with one clue.",
    science: "Kids claim + evidence in kid words.",
    social: "Kids share one clear region / map fact.",
  };
  return bySub[act?.subject] || "Kids show the idea in a short artifact.";
}
...
export function getProjectShell(id) {
  ...
  return {
    ...
    whoNeedsMeHref: "/v2/teacher/check-ins",
    backHref: DAY_FOCUS_HREF(act.day),
    source: act.source,
    materialsStub: materialsStub(act, span),
...
/**
 * Resolve student project Open id → shell model.
 * Accepts mission id (`project-act-8`) or teacher activity id (`act-8`).
 */
export function getStudentProjectShell(rawId) {
  if (!rawId) return null;
  const decoded = decodeURIComponent(String(rawId));
  const activityId = decoded.startsWith("project-")
    ? decoded.slice("project-".length)
    : decoded;

  const teacherShell = getProjectShell(activityId);
  const assigned = loadAssignedProjects().find(
    (p) => p.id === activityId || `project-${p.id}` === decoded
  );

  if (!teacherShell && !assigned) return null;

  const base = teacherShell || {
    id: activityId,
    title: assigned?.title || "Project",
    subject: assigned?.subject,
    subjectName: assigned?.subject || "Subject",
    subjectColor: "#8B6CFF",
    standard: assigned?.standard || "",
    teksLabel: assigned?.standard ? `TEKS ${assigned.standard}` : "TEKS stub",
    minutes: assigned?.minutes || 20,
    day: assigned?.day != null ? assigned.day : 2,
    evidence: {
      label: assigned?.evidenceLabel || "Short artifact",
      title: assigned?.evidenceTitle || "Evidence product",
      body: "Turn in your evidence when you're ready — stub only.",
    },
    checkpoints: [
      { dayLabel: "Day 1", beat: "Start your evidence draft." },
      { dayLabel: "Day 2", beat: "Add one clear detail." },
      { dayLabel: "Day 3", beat: "Polish and submit." },
    ],
    samLine: "Multi-day project — take it one checkpoint at a time.",
    span: { chip: "multi-day", label: "Multi-day" },
  };

  const evidenceLabel =
    assigned?.evidenceLabel ||
    base.evidence?.label ||
    "Short artifact";
  const audienceId = assigned?.audienceId || "whole_class";
  ...
  return {
    ...
    teksLabel: base.teksLabel,
    minutes: base.minutes || 20,
    day: base.day,
    samLine: base.samLine || "Take it one checkpoint at a time.",
    spanChip: base.span?.chip || "multi-day",
    evidenceLabel,
    evidenceTitle: base.evidence?.title || "Evidence product",
    evidenceBody:
      base.evidence?.body ||
      `Turn in your ${String(evidenceLabel).toLowerCase()} when ready.`,
    checkpoints: Array.isArray(base.checkpoints) ? base.checkpoints : [],
    audienceId,
    audienceLabel: assigned?.audienceLabel || (audienceId === "small_group" ? "Small group" : "Whole class"),
    backHref: "/v2/student",
    product: assigned?.product || base.product || "ClearLessons",
  };
}
