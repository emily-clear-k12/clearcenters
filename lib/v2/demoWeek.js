...
export const DEMO_TEACHER = { name: "Mrs. Barrons" };
...
/**
 * Demo morning-card insights for Daily Focus (skeleton).
 * Wins first emotionally; watch stays calm "not yet" + next step.
 * Prefer class-specific rows when period filter matches.
 */
export const DEMO_MORNING_INSIGHTS = [
  {
    subject: "math",
    classKey: "B",
    win: () => "Period 2 grew on fraction strips yesterday — 20 of 24 locked equivalents on a number line.",
    watch: () => "4 kids still mix numerator and denominator — not yet. Next step: a 5-min models warm-up before teach.",
  },
  {
    subject: "math",
    classKey: "A",
    win: () => "Period 1 nailed the exit-ticket warm-up — almost everyone compared fractions with a picture.",
    watch: () => "A couple of students aren't solid on equivalent fractions yet. Next step: pair them during The Pizza Problem.",
  },
  {
    subject: "math",
    classKey: "C",
    win: () => "Period 3's number-line check-in clicked — those four left more sure of themselves.",
    watch: () => "A few still guess on equivalents — not alarming. Next step: keep the clinic short and visual.",
  },
  {
    subject: "math",
    classKey: null,
    win: (cls) => `${cls || "Your class"} grew on fraction strips yesterday — most kids placed equivalents without a struggle.`,
    watch: (cls) => `A few in ${cls || "the room"} still swap numerator and denominator — calm not-yet. Next step: models warm-up before teach.`,
  },
  {
    subject: "elar",
    classKey: "A",
    win: () => "Period 1 reading logs showed real author's-purpose language showing up in their own words.",
    watch: () => "5 students could use another pass on persuade vs inform — not yet. Next step: Purpose match stations today.",
  },
  {
    subject: "elar",
    classKey: null,
    win: (cls) => `${cls || "Your readers"} got traction on The Great Garden — purpose talk sounded sharper.`,
    watch: (cls) => `A small group in ${cls || "the room"} isn't firm on author's purpose yet. Next step: a quick reteach chip later.`,
  },
  {
    subject: "science",
    classKey: "A",
    win: () => "Period 1 density predictions were thoughtful — kids argued with evidence, not guesses.",
    watch: () => "3 students aren't there yet on density (5.6A). Next step: short reteach before mixtures.",
  },
  {
    subject: "science",
    classKey: null,
    win: (cls) => `${cls || "Your class"} owned Sink or Float yesterday — predictions got better mid-lab.`,
    watch: (cls) => `A few in ${cls || "the room"} still mix mixture vs solution — not yet. Next step: lab notebook check today.`,
  },
  {
    subject: "social",
    classKey: null,
    win: (cls) => `${cls || "Your class"} mapped the Gulf Coast with clearer region language than Monday.`,
    watch: (cls) => `Map skills are still wobbly for a few in ${cls || "the room"} — calm not-yet. Next step: Map Detectives today.`,
  },
];

function pickMorningInsight(pool, subject, classKey) {
  const forSubject = pool.filter((r) => r.subject === subject);
  const exact = forSubject.find((r) => r.classKey === classKey);
  if (exact) return exact;
  const general = forSubject.find((r) => r.classKey == null);
  return general || forSubject[0] || null;
}

/** Build morning-card copy from setup / period / subject filters + today's Now item. */
export function buildMorningCardDemo({ subjects, classKey, className, subjectFilter, agendaNow }) {
  const subjectPool =
  ...
  // Prefer the Now activity's subject when it matches the filter pool
  const nowSubject = agendaNow?.subject && subjectPool.includes(agendaNow.subject) ? agendaNow.subject : subjectPool[0];
  const insight = pickMorningInsight(DEMO_MORNING_INSIGHTS, nowSubject, classKey)
    || pickMorningInsight(DEMO_MORNING_INSIGHTS, subjectPool[0], classKey)
    || DEMO_MORNING_INSIGHTS[0];

  const room = className || "this class";
  let agendaLine = `Steady day ahead for ${room}.`;
  if (agendaNow) {
    const sub = SUBJECTS[agendaNow.subject];
    const kind = KINDS[agendaNow.kind]?.short || agendaNow.kind;
    const label = agendaNow.kind === "teach" ? "Main teach" : "Now";
    agendaLine = `${label}: ${agendaNow.title} · ${sub?.name || agendaNow.subject} · ${kind}`;
  }

  const greeting = `Good morning — here's your open for ${room}.`;

  // Loop seam: story readiness + soft who (same cast as Reports), not count/% chrome.
  // Lazy require — demoLoopSeams → demoGrading → demoWeek. Top-level import TDZ's on
  // Vercel prerender ("Cannot access 'l' before initialization" in shared chunk).
  const { buildFocusWinSeam, buildFocusWatchSeam } = require("./demoLoopSeams");
  const fallbackWin = typeof insight.win === "function" ? insight.win : () => insight.win;
  const win = buildFocusWinSeam({
    subject: insight.subject || nowSubject,
    className,
    fallbackWin,
  });
  const watchSeam = buildFocusWatchSeam({
    subject: insight.subject || nowSubject,
    className,
  });
  ...
  return {
    ...
    agendaLine,
    win,
    watch: watchSeam.watch,
    subject: insight.subject,
    readiness: watchSeam.readiness || null,
    softWho: watchSeam.whoLine || null,
    softestHref: watchSeam.softestHref || null,
    softestCode: watchSeam.softestCode || null,
    reportsHref: watchSeam.reportsHref || null,
  };
}
