// CI2.0 sandbox — pretend data for the teacher "This Week" planner.
// Nothing here touches the database. Once the look and flow are approved,
// this is replaced by real data from unit plans, routines and results.

export const SUBJECTS = {
  math: { key: "math", name: "Math", color: "#3D84F5", unit: "Fractions" },
  elar: { key: "elar", name: "ELAR", color: "#C94FD6", unit: "Author's Purpose" },
  science: { key: "science", name: "Science", color: "#0FA3B1", unit: "Matter & Energy" },
  social: { key: "social", name: "Social Studies", color: "#7C4DDB", unit: "Texas Regions" },
};
...
// Sandbox-only "Preview as" setups, to show how the same planner fits
// different teaching assignments.
export const TEACHER_SETUPS = {
  self: {
    label: "Self-contained · all subjects",
    subjects: ["math", "elar", "science", "social"],
    classes: [{ key: "A", name: "Room 12" }],
  },
  math: {
    label: "Math only · 3 classes",
    subjects: ["math"],
    classes: [
      { key: "A", name: "Class A" },
      { key: "B", name: "Class B" },
  ...
  elarss: {
    label: "ELAR + Social Studies · 2 classes",
  ...
  scisoc: {
    label: "Science + Social Studies · 3 classes",
...
// classes: "all" = planned once for every class; otherwise specific class keys.
let n = 0;
...
// SAM suggestions appear beside the subject and day they belong to.
// classKey: the class the suggestion is about (falls back to the teacher's
// first class if that class doesn't exist in the current setup).
export const DEMO_SUGGESTIONS = [
  {
    ...
    day: 2,
    classKey: "A",
    text: (cls) => `3 students${cls ? ` in ${cls}` : ""} aren't there yet on 5.6A (density).`,
    action: "Add reteach",
  ...
  },
  {
    ...
    day: 3,
    classKey: "C",
    text: (cls) => `5 students${cls ? ` in ${cls}` : ""} are mixing up numerators and denominators.`,
...
export const HANDS_OFF_LEVELS = [
  { key: "i_plan", title: "I'll plan it", body: "The site suggests activities, and you build the week yourself." },
  { key: "plan_for_me", title: "Plan it for me, I'll publish", body: "Your week is built from your unit plan and routines. You review it and click Publish." },
  { key: "run_for_me", title: "Run it for me", body: "Your routines assign automatically. You get a Sunday preview and can change anything first." },
];
