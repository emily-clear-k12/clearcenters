...
export const DEMO_ASSIGNMENTS = [
    ...
    bands: [
      {
        ...
        count: 12,
        tone: "ready",
        names: ["Liam", "Ava", "Emma", "Priya", "Owen", "Jordan"],
      },
      {
        key: "almost",
        label: "Almost there",
  ...
  },
  {
    ...
    when: "2 days ago",
    softSummary:
      "When the models matched, reasons were solid. When they didn’t, Maya and Zoe went quiet or guessed.",
    calmNext:
      "Pair share with matching models first, then Check-ins for Maya · Zoe if they’re still stuck.",
    bands: [
      {
        key: "clear",
        label: "Looking clear",
  ...
  },
  {
    ...
    calmNext: "On Today: mark + say why in one sentence. Keep an eye on Maya.",
    bands: [
      {
        key: "clear",
        label: "Looking clear",
  ...
  },
  {
    ...
    when: "Last week",
    softSummary:
      "Models were mostly clear. Maya and Jordan swapped parts on one pair — worth a soft look if 4.3C stays soft.",
    calmNext:
      "Quick visual reteach on Today if the shaky rows linger.",
    bands: [
      {
        key: "clear",
        label: "Looking clear",
...
/** Format a soft name list for UI (Maya · Zoe · Diego). */
/** Glance label: plain skill words first; code only as quiet backup. */
export function shortSkillLabel(standard) {
  if (!standard) return "this skill";
  const plain = String(standard.plain || "").trim();
  if (plain) {
    const head = plain.split(/[—–]/)[0].split(" - ")[0].trim().replace(/\.$/, "");
    if (head) return head;
  }
  return standard.code ? `TEKS ${standard.code}` : "this skill";
}

/** Colleague line: plain skill · optional code in parentheses when useful. */
export function skillGlanceLabel(standard, { withCode = false } = {}) {
  const short = shortSkillLabel(standard);
  if (withCode && standard?.code && !String(short).includes(String(standard.code))) {
    return `${short} (${standard.code})`;
  }
  return short;
}
...
export function buildSamStory(standard, leadAssignment = null) {
  if (!standard) return "Reports aren’t ready yet.";
  ...
  const skill = skillGlanceLabel(standard);
  const lookBit =
  ...
  if (standard.tone === "needsYou" && who) {
    return `Needs a look: ${skill}. ${lookBit} Watch ${who} — ${hint}.${workBit}`;
  }
  if (who) {
    return `${skill} — ${lookBit} Keep an eye on ${who}.${workBit}`;
  }
  return `${skill} — ${lookBit}${workBit}`;
}
...
export function getStandardDetail(code) {
  ...
  const calmNext =
    standard.tone === "needsYou"
      ? whoLine
        ? `Next: Check-ins for ${whoLine} on ${skillGlanceLabel(standard)}, then a short reteach on Today.`
        : `Next: Check-ins for anyone still stuck on ${skillGlanceLabel(standard)}, then a short reteach on Today.`
      : `Next: keep Today’s pace on ${skillGlanceLabel(standard)}; Check-ins only if someone flags.`;
  return {
    standard,
    contributing,
    softHint,
...
export function getReportsStub() {
  ...
  const samStory = softest
    ...
    : "Reports aren’t ready yet — Check-ins when someone needs you.";
  const samLine = softest
    ? `Needs you · ${skillGlanceLabel(softest)}${
        softest.softCluster?.length
          ? ` · ${formatSoftNames(softest.softCluster, 3)}`
          : ""
      }${softHint ? ` — ${softHint}` : ""}.`
    : "Standards look calm — Check-ins when someone needs you.";
  ...
  return {
    ...
    softestNextMoveLabel: softest
      ? `Open ${skillGlanceLabel(softest)} report`
      : "Open Reports",
    softestReteachLabel: softest
      ? `Plan a quick reteach · ${skillGlanceLabel(softest)}`
