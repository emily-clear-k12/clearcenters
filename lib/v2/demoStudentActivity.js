...
import { resolvePracticeAssignment } from "./demoLiveTeach";
import { DEMO_TEACHER } from "./demoWeek";

/** Base localStorage key — scoped per demo kid (soft cast + Leo / Kai / Riley). */
...
export function resolveStudentMission(missionId) {
  ...
  if (missionId === SOFT_MIXTURES_MISSION.id) return { ...SOFT_MIXTURES_MISSION };
  ...
  // Practice assigned from Teach live (same browser).
  if (String(missionId).startsWith("practice-") || resolvePracticeAssignment(missionId)) {
    const entry = resolvePracticeAssignment(missionId);
    if (entry) {
      return {
...
export function readStudentProgress() {
  ...
  try {
    const raw = readStudentScopedRaw(STUDENT_PROGRESS_KEY);
...
export function markMissionDone(missionId) {
  ...
  try {
    writeStudentScopedRaw(STUDENT_PROGRESS_KEY, JSON.stringify(next));
    try {
      window.dispatchEvent(
        new CustomEvent("ci2-student-progress-updated", {
          detail: { missionId, doneIds },
        })
      );
    } catch {
      /* ignore */
...
const ACTIVITY_BY_ID = {
  "stu-now-equiv": {
    directions: `Warm up for ${DEMO_TEACHER.name}’s lesson. Tap the fraction that matches ½ — then you’re ready.`,
    items: [
      {
        id: "q1",
        kind: "mc",
  ...
  "stu-soft-mixtures": {
    directions:
      "TEKS 5.6B · Mixtures vs solutions. Make one claim and one evidence note — same soft story as class.",
    items: [
      {
        id: "q1",
        kind: "mc",
        prompt: "Salt disappears in water. Sand stays visible. Which is the solution?",
        choices: [
          { id: "a", label: "Salt water" },
          { id: "b", label: "Sand in water" },
          { id: "c", label: "Both the same" },
        ],
        correctId: "a",
      },
      {
        id: "q2",
        kind: "short",
        prompt: "Claim + one evidence note: is trail mix a mixture or a solution? Why?",
        placeholder: "Claim: … Evidence: …",
      },
    ],
    samSteps: [
      {
        label: "Nudge",
        text: "Not yet — ask: can you still see the parts? If yes, it’s usually a mixture.",
      },
      {
        label: "Hint",
        text: "Solutions look even — the solute seems gone. Mixtures keep bits you can spot or separate.",
      },
      {
        label: "Example",
        text: "Example: salt water = solution (salt gone). Trail mix = mixture (nuts still there). Add one evidence note.",
      },
    ],
  },
};
