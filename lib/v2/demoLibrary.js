...
/** Mrs. Barrons demo · same-browser Library heart favorites (card ids). */
export const LIBRARY_FAVORITES_KEY = "ci2.teacher.libraryFavorites";
...
export function saveLibraryFavorites(ids) {
  if (typeof window === "undefined") return;
  try {
    const list = Array.isArray(ids) ? ids.filter((x) => typeof x === "string") : [];
    window.localStorage.setItem(LIBRARY_FAVORITES_KEY, JSON.stringify(list.slice(0, 80)));
  } catch {
    /* ignore quota */
  }
}

/** Toggle heart; returns next id list. */
export function toggleLibraryFavorite(cardId) {
  if (!cardId) return loadLibraryFavorites();
  const prev = loadLibraryFavorites();
  const next = prev.includes(cardId) ? prev.filter((id) => id !== cardId) : [cardId, ...prev];
  saveLibraryFavorites(next);
  return next;
}
...
/** Current teacher period/room from localStorage (departmentalized filter). */
function readTeacherClassFilter() {
  if (typeof window === "undefined") return null;
  try {
    const ck = window.localStorage.getItem("ci2.teacher.classFilter");
    return ck && String(ck).trim() ? String(ck).trim() : null;
  } catch {
    return null;
  }
}
...
/**
 * Stamp classes for minted library tiles so room filters show them in the right period.
 * Explicit classKey wins; else current ci2.teacher.classFilter; "all" stays all-rooms.
 */
export function resolveLibraryClasses(classKey = null) {
  const key = classKey != null && classKey !== "" ? classKey : readTeacherClassFilter();
 ...
 * Stamps current class/period so departmentalized filters show it in the right room.
 * Provenance stays From Library (fromLibrary + added-lib- id).
 * opts.classKey — optional override; default reads ci2.teacher.classFilter.
 */
export function addLibraryCardToPlanner(card, target = "daily_focus", opts = null) {
  if (typeof window === "undefined" || !card) return null;
  const day = target === "this_week" ? FOCUS_TODAY_DAY : FOCUS_TODAY_DAY;
  const classKey = opts && opts.classKey != null ? opts.classKey : null;
  const classes = resolveLibraryClasses(classKey);
  const entry = {
