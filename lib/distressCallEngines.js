...
export const DISTRESS_CALL_ENGINES = ["mission_map", "simulation_lab", "fact_check_desk", "relay_station", "assembly_deck"];
...
export function distressCallUnit(engine, standard) {
  if (engine === "relay_station") {
    const code = String(standard || "");
    return /\.TRACK$/.test(code) ? "levels passed" : /\.DAILY$/.test(code) ? "daily transmissions" : "stars";
  }
  if (engine === "assembly_deck") return "sentences placed";
  return "checkpoints";
}
