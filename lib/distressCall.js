...
// Relay Station (Sept 22 2026). The submit route mirrors a summary onto the
// assignment's submission row: for the Foundations Track that's
// { track: true, passedCount }, so the meter counts levels passed; for a
// single reading it's { best: { stars } }, so the meter counts stars. Both
// are instant, deterministic grades — no teacher scoring involved.
function relayStationProgress(row) {
  const data = row.relay_station_data;
  if (!data) return null;
  if (data.track) return { correct: data.passedCount || 0, total: 20 };
  if (data.dailyMirror) return { correct: data.totalDays || 0, total: data.totalDays || 1 };
  if (data.best) return { correct: data.best.stars || 0, total: 3 };
  return null;
}
...
// Assembly Deck (Sept 22 2026). The meter counts sentences placed correctly
// across every paragraph in the case — the part that is instantly graded.
function assemblyDeckProgress(row) {
  const data = row.assembly_deck_data;
  if (!data || !data.placement) return null;
  return { correct: data.placement.correct || 0, total: data.placement.total || 0 };
}
...
const ENGINE_ADAPTERS = {
  ...
  relay_station: relayStationProgress,
  assembly_deck: assemblyDeckProgress,
};
...
export async function computeDistressCallProgress(assignmentId) {
  ...
  const { data: submissions } = await supabaseAdmin
    .from("submissions")
    .select("student_id, mission_map_data, simulation_lab_data, signal_data, relay_station_data")
    .eq("assignment_id", assignmentId)
  ...
  // Wording for the meter ("12 / 40 levels passed" instead of "checkpoints").
  const unitLabel =
    engine === "relay_station"
      ? (/\.TRACK$/.test(String(assignment.case_standard || "")) ? "levels passed" : /\.DAILY$/.test(String(assignment.case_standard || "")) ? "daily transmissions" : "stars earned")
      : "checkpoints cleared";
