...
export async function getVisibleAssignmentsForStudent(studentId, classId) {
  const { data: rawAssignments } = await supabaseAdmin
    .from("assignments")
    .select("id, due_date, case_standard, created_at, cases(title, learning_target)")
    .eq("class_id", classId)
  ...
  if (assignmentIds.length > 0) {
    const [{ data: targeting }, { data: completed }] = await Promise.all([
      ...
      // finished and turned in, not ones they've merely started.
      supabaseAdmin
        .from("submissions")
  ...
  const completedAssignmentIds = new Set(completedRows.map((s) => s.assignment_id));
