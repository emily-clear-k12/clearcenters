...
export const COLORS = {
  ...
  // read as "off-putting" per Emily's feedback. Went with a bright pink next
  // (`#FF6FA0`), which Emily also didn't like — replaced same day with this
  // copper/amber instead. `copper` is Mission Control's own color now, used
  // only for decoration (HUD dot, buttons, panel borders); `warning` stays
  // reserved purely for real urgency signals.
  copper: "#E8935A",
...
export const PAGE_ACCENTS = {
  "/teacher/assign": COLORS.copper, // Mission Control (see `copper` note above)
  ...
  // Student detail isn't its own Overview landmark either — it's reached
  // from Grading and Reports (both aqua Observatory), so it shares that
  // family rather than inventing a color for a page nothing points to
  // directly from Overview.
  "/teacher/students": COLORS.aqua,
  ...
  // Roster isn't its own Overview landmark — it's reached from My Classes'
  // "Manage Roster" button — so it shares Mission Control's family rather
  // than inventing a 6th accent for a page that's really an extension of
  // class management.
  "/teacher/roster": COLORS.copper,
};
...
export const PAGE_BACKGROUNDS = {
  ...
  "/teacher/students": "/teacher/console/bg-observatory.jpg",
  ...
  // family accent (copper) as its parent page, just its own background.
  "/teacher/assign/new": "/teacher/challenge_library_bg.jpg",
  // Same room art as My Classes — Roster is really an extension of class
  // management, not a destination of its own.
  "/teacher/roster": "/teacher/console/bg-platform-room.jpg",
};
