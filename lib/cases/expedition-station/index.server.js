...
export function gradeExpeditionTask(standard, taskId, body) {
  ...
  if (ans.kind === "mistake") {
    ...
    return {
      ...
      stars: ok ? starsFor(wrongTries, usedHint) : 0,
      hint: ok ? null : hint,
      feedback: ok ? "Fixed! 3/5 + 1/5 = 4/5. The parts stay fifths." : "Not yet. Try again.",
    };
  }
  ...
  if (ans.kind === "numberless") {
    ...
    return {
      ...
      stars: ok ? starsFor(wrongTries, usedHint) : 0,
      hint: ok ? null : wrongHint(task, attempt),
      feedback: ok ? "The tank is full. The crew has water for the storm!" : "Not yet. Try again.",
    };
  }
  ...
  if (ans.kind === "steps") {
    ...
    return {
      ...
      hint: ok ? null : wrongHint(task, attempt),
      feedback: ok ? (last ? "Challenge complete!" : `Step ${step} is right. On to step ${step + 1}.`) : "Not yet. Try again.",
    };
  }
  ...
  if (ans.kind === "compare") {
    ...
    return {
      ...
      feedback: ok ? "Right! Kai hiked 5/6 km, which is 1/6 km farther than Nova." : "Not yet. Try again.",
    };
  }
  ...
  if (ans.kind === "sort") {
    ...
    return {
      ...
      feedback: ok ? "Every pack is in the right bin. The strongest packs go to the tower!" : `${wrongItems.length} pack${wrongItems.length === 1 ? " is" : "s are"} in the wrong bin.`,
    };
  }
