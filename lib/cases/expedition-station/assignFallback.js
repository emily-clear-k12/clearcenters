...
/** True when a library row will pass Grade/Subject/engine Assign filters for Frozen Relay. */
export function isPlayableFrozenRelayRow(row) {
  if (!row || typeof row !== "object") return false;
  const n = normalizeCaseRow(row);
  return (
    n.standard === FROZEN_RELAY_STANDARD &&
    n.engine === "expedition_station" &&
    Number(n.grade) === 4 &&
    n.subject === "Math"
  );
}
