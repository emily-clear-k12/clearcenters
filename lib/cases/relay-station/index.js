...
export const PASS_ACCURACY = 90; // accuracy needed to pass a track level
...
READINGS.forEach((r) => {
  ...
  LESSONS[r.code] = {
    ...
    newKeys: [],
    standard: r.standard,
    text: segments ? segments.map((s) => s.text).join("") : r.text,
    segments,
    goals: READING_GOALS[r.grade],
  };
});
...
export function computeRun({ chars, keystrokes, errors, ms }) {
  ...
  return {
    ...
    accuracy: Math.round(((safeKeys - errors) / safeKeys) * 100),
  };
}
