// Maker Studio Wave 1–3 — teacher-reviewed artifacts (NOT AI-graded).
import { getMakerStudioCase, resolveMakerConfig } from "./catalog.js";
import { modeById } from "./modes.js";

function slotPreview(id, slot) {
  if (!slot || typeof slot !== "object") return "";
  if (id === "write") return typeof slot.text === "string" ? slot.text : "";
  if (id === "sketch") return slot.imageDataUrl ? "(sketch drawing)" : "";
  if (id === "diagram") {
    const cap = typeof slot.caption === "string" ? slot.caption.trim() : "";
    if (cap) return cap;
    return slot.imageDataUrl ? "(diagram drawing)" : "";
  }
  if (id === "poster") {
    const title = typeof slot.title === "string" ? slot.title.trim() : "";
    const caption = typeof slot.caption === "string" ? slot.caption.trim() : "";
    const bits = [title, caption].filter(Boolean);
    if (bits.length) return bits.join(" — ");
    return slot.imageDataUrl ? "(poster image)" : "";
  }
  if (id === "comic") {
    const panels = Array.isArray(slot.panels) ? slot.panels : [];
    const lines = panels
      .map((p, i) => {
        const t = p && typeof p.text === "string" ? p.text.trim() : "";
        return t ? `Panel ${i + 1}: ${t}` : null;
      })
      .filter(Boolean);
    if (lines.length) return lines.join("\n");
    return panels.some((p) => p && p.imageDataUrl) ? "(comic panels)" : "";
  }
  if (id === "voice") {
    if (slot.audioDataUrl) {
      const sec = Number(slot.durationSec);
      return Number.isFinite(sec) && sec > 0
        ? `(voice note · ${Math.round(sec)}s)`
        : "(voice note)";
    }
    return "";
  }
  if (id === "before_after") {
    const cap = typeof slot.caption === "string" ? slot.caption.trim() : "";
    const hasB = !!(slot.before && slot.before.imageDataUrl);
    const hasA = !!(slot.after && slot.after.imageDataUrl);
    if (cap) return cap;
    if (hasB || hasA) return "(before/after images)";
    return "";
  }
  if (id === "map_it") {
    const cap = typeof slot.caption === "string" ? slot.caption.trim() : "";
    const pins = Array.isArray(slot.pins) ? slot.pins : [];
    const labels = pins
      .map((pin) => (pin && typeof pin.label === "string" ? pin.label.trim() : ""))
      .filter(Boolean);
    if (cap && labels.length) return `${cap} · pins: ${labels.join(", ")}`;
    if (cap) return cap;
    if (labels.length) return `pins: ${labels.join(", ")}`;
    return slot.imageDataUrl ? "(map)" : "";
  }
  if (id === "math_story") {
    const story = typeof slot.story === "string" ? slot.story.trim() : "";
    const work = typeof slot.workText === "string" ? slot.workText.trim() : "";
    const bits = [story, work].filter(Boolean);
    if (bits.length) return bits.join("\n");
    return slot.imageDataUrl ? "(math story work)" : "";
  }
  if (id === "interview") {
    const rows = Array.isArray(slot.rows) ? slot.rows : [];
    const lines = rows
      .map((r, i) => {
        const q = r && typeof r.question === "string" ? r.question.trim() : "";
        const a = r && typeof r.answer === "string" ? r.answer.trim() : "";
        if (!q && !a) return null;
        return `Q${i + 1}: ${q || "(question)"}\nA: ${a || "(no answer)"}`;
      })
      .filter(Boolean);
    return lines.join("\n");
  }
  if (id === "teach_the_buddy") {
    const expl = typeof slot.explanation === "string" ? slot.explanation.trim() : "";
    const qs = Array.isArray(slot.questions) ? slot.questions : [];
    const lines = qs
      .map((q, i) => {
        const qq = q && typeof q.question === "string" ? q.question.trim() : "";
        const aa = q && typeof q.answer === "string" ? q.answer.trim() : "";
        if (!qq && !aa) return null;
        return `Q${i + 1}: ${qq || "(question)"}\nA: ${aa || "(no answer)"}`;
      })
      .filter(Boolean);
    const bits = [expl, ...lines].filter(Boolean);
    return bits.join("\n");
  }
  if (id === "paint_what_i_said") {
    const p = typeof slot.promptText === "string" ? slot.promptText.trim() : "";
    if (p) return p;
    return slot.imageDataUrl ? "(painted picture)" : "";
  }
  if (id === "what_if") {
    const twist = typeof slot.twist === "string" ? slot.twist.trim() : "";
    const ending = typeof slot.ending === "string" ? slot.ending.trim() : "";
    const beats = Array.isArray(slot.beats)
      ? slot.beats.map((b) => String(b || "").trim()).filter(Boolean)
      : [];
    const bits = [];
    if (twist) bits.push(`What if ${twist}?`);
    beats.forEach((b, i) => bits.push(`Beat ${i + 1}: ${b}`));
    if (ending) bits.push(`Ending: ${ending}`);
    return bits.join("\n");
  }
  if (id === "postcard") {
    const to = typeof slot.to === "string" ? slot.to.trim() : "";
    const from = typeof slot.from === "string" ? slot.from.trim() : "";
    const msg = typeof slot.message === "string" ? slot.message.trim() : "";
    const bits = [];
    if (to) bits.push(`To: ${to}`);
    if (from) bits.push(`From: ${from}`);
    if (msg) bits.push(msg);
    if (bits.length) return bits.join("\n");
    return slot.imageDataUrl ? "(postcard)" : "";
  }
  if (id === "sort_of_my_own") {
    const cats = Array.isArray(slot.categories) ? slot.categories : [];
    const items = Array.isArray(slot.items) ? slot.items : [];
    const lines = cats.map((c, i) => {
      const label = typeof c === "string" ? c.trim() : "";
      const members = items
        .filter((it) => it && Number(it.categoryIndex) === i)
        .map((it) => (typeof it.label === "string" ? it.label.trim() : ""))
        .filter(Boolean);
      return `${label || `Category ${i + 1}`}: ${members.join(", ") || "(empty)"}`;
    });
    return lines.join("\n");
  }
  return typeof slot.text === "string" ? slot.text : "";
}

export function summarizeMakerPieces(data, config) {
  const modes = (data && data.modes) || {};
  const enabled = (config && config.enabledModes) || ["write"];
  const pieces = enabled.map((id) => {
    const slot = modes[id] || {};
    const meta = modeById(id);
    return {
      id,
      label: (meta && meta.label) || id,
      status: slot.status || "empty",
      text: slotPreview(id, slot),
      imageDataUrl: typeof slot.imageDataUrl === "string" ? slot.imageDataUrl : null,
      audioDataUrl: typeof slot.audioDataUrl === "string" ? slot.audioDataUrl : null,
      title: typeof slot.title === "string" ? slot.title : "",
      caption: typeof slot.caption === "string" ? slot.caption : "",
      panels: Array.isArray(slot.panels) ? slot.panels : null,
      durationSec: slot.durationSec != null ? Number(slot.durationSec) : null,
      updatedAt: slot.updatedAt || null,
      raw: slot,
    };
  });
  const doneCount = pieces.filter((p) => p.status === "done").length;
  // Submit when every enabled mode is Done (finishN mirrors enabledModes.length).
  return { pieces, doneCount, finishN: Math.max(1, enabled.length || 1) };
}

export function makerAttemptText(data, config) {
  const { pieces } = summarizeMakerPieces(data, config);
  if (!pieces.length) return "(no pieces yet)";
  return pieces
    .map((p) => {
      const body = (p.text || "").trim() || "(empty)";
      return `[${p.label} · ${p.status}]\n${body}`;
    })
    .join("\n\n");
}

export function getMakerStudioCaseOrNull(standard) {
  return getMakerStudioCase(standard);
}

export { resolveMakerConfig };
