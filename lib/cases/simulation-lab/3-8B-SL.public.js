...
export const PUBLIC_CASE = {
  ...
  pretrialSteps: {
    ...
    chooseVariables: {
      ...
      keepTheSame: [
        ...
        "Same ramp and same floor every trial (Round 1)",
  ...
  // right after the Round 1 checkpoints. Sept 24, 2026: the twist changed
  // from "heavier ball" to a ROUGHER FLOOR (carpet laid over the tiles).
  // The heavier-ball version taught bad physics (a heavier ball on the same
  // ramp does not really roll a shorter distance); carpet adds friction,
  // which really does stop the cart sooner. Same increasing trend (steeper
  // still means farther, never shorter) but every distance is shorter than
  // Round 1's at the same angle. See design doc §10.2 (point 1) and §10.5.
  //
  // TO REVERT to the heavier ball: set roundTwoLabel back to "heavier
  // ball", restore the conditionChangeDescription and the table in the
  // comment below, set scene.twist.type to "heavyBall" (+ its question),
  // swap the fair-test "floor" choice back to "surface"/"What the ramp is
  // made of", and restore aiContext in 3-8B-SL.server.js.
  //   heavier ball: "Mission Control just swapped in a heavier cargo ball.
  //   Same ramp, same angles — but does the pattern you found still hold?"
  //   table: 10->1, 15->2, 20->3, 25->4, 30->5, 35->6, 40->7, 45->8, 50->9
  // Short label the console/trial-log headers show next to "ROUND 2" —
  // added when this became a multi-case engine (the old client's
  ...
  roundTwo: {
    conditionChangeDescription:
      "Mission Control just rolled carpet over the floor tiles. Same ramp, same ball, same angles — but does the pattern you found still hold?",
    // Rougher floor: shorter than Round 1 at every angle, and never shorter
    // at a steeper angle (non-decreasing).
    lookupTable: [
      { angle: 10, distance: 1 },
      { angle: 15, distance: 2 },
      { angle: 20, distance: 2 },
  ...
  checkpoints: [
    ...
    // route never scores it for that client.
    {
      ...
      choices: [
        { id: "ball", text: "Which ball we used", icon: "ball" },
        { id: "floor", text: "What the floor is made of", icon: "floor" },
        { id: "angle", text: "How steep the ramp was", icon: "angle" },
        { id: "start", text: "Where the cart started", icon: "start" },
      ],
    },
    {
      id: "cp2",
  ...
  dataTableStep: {
    ...
    tolerance: 1,
    instructions:
      "You've tested some angles on the carpet floor, but not all of them. Pick an angle you never tried this round, and predict how far the ball would roll — use the pattern in your own data to make your best prediction.",
  },
  ...
  // exact image's layout (the old client's CONSOLE_HOTSPOTS
  ...
  scene: {
    ...
    unitOne: "tile",
    outcomeAxis: "Tiles",
    variableAxis: "Ramp angle",
    checkpointIds: ["cp1", "fair"],
    choiceIcons: { cp1: { increase: "up", decrease: "down", same: "flat" } },
    twist: {
      type: "carpet", // "heavyBall" = the previous twist (see the Round 2 note above)
      banner: "Conditions change!",
      question: "The floor has carpet on it now. Does your pattern still hold?",
    },
  },
  ...
  generalizePrompt:
    "What happens to the distance the ball rolls as the ramp gets steeper? Back it up with one real trial from your chart — an angle you tried and the distance you saw.",
