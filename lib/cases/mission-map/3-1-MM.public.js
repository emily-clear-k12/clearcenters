...
// Unlock requirement.
//
// NOTE ON THE CASE CODE: "3.1" is this library's own internal concept
// number, NOT a state standard code — the real standard this case teaches
// is in `teksLabel` below (TEKS 3.12B), confirmed in the Aug 31 v6 audit
// described further down. The two are different numbers, which is true of
// almost every Mission Map case. See lib/cases/mission-map/teksLabels.js —
// that map is what the teacher library, the student header and the
// Standards report read, so a teacher sees the real standard rather than
// this internal code.
//
// (An older version of this comment said "TEKS NOT YET VERIFIED"; that was
// left behind by the v6 audit below, which did verify it.)
//
...
export const PUBLIC_CASE = {
  ...
  tagline: "Fewer bees are visiting a garden. Find out why that matters.",

  mission: {
    briefText:
      "The flowers in a local garden are not making seeds like they used to. The garden club also sees fewer bees. Walk the garden path and gather evidence at each stop. Find out how this garden's food chain works. Then find out what happens when the bees go away.",
    goal: "Explain how energy moves through this garden's food chain. Tell what happens to the rest of the chain when there are fewer bees.",
  },
  ...
  checkpoints: [
    {
      ...
      order: 1,
      position: { x: 12, y: 75 },
      prompt: "Gate 1: What does a food chain in this garden look like?",
      evidence: {
        type: "passage",
      ...
      choices: [
        { id: "a", text: "Bees, then flowers, then the Sun" },
        { id: "b", text: "Flowers only need bees. They don't need the Sun" },
        { id: "c", text: "Sun, then flowers (producers), then bees (consumers)" },
    ...
    },
    {
      ...
      order: 2,
      position: { x: 28, y: 45 },
      prompt: "Gate 2: Which clue shows energy moving from a flower to a bee?",
      evidence: {
        type: "data",
      ...
      choices: [
        { id: "a", text: "The ladybug eating aphids on the leaf" },
        { id: "b", text: "The bee feeding on nectar from the flower" },
        { id: "c", text: "The butterfly resting on the rock" },
        { id: "d", text: "The rabbit hopping through the grass" },
      ],
      correctChoiceId: "b",
      evidenceLogEntry: "The trail camera caught a bee drinking nectar. That is the flower-to-bee link.",
    },
    {
      id: "cp3",
      order: 3,
      position: { x: 45, y: 68 },
      prompt: "Gate 3: Two volunteers checked the garden three weeks later. They do not agree. Which note matches the evidence?",
      evidence: {
        type: "passage",
      ...
      choices: [
        { id: "a", text: "Volunteer B's note — flowers don't need bees" },
        { id: "b", text: "Both notes could be true at the same time" },
        { id: "c", text: "Neither note counts as real evidence" },
        { id: "d", text: "Volunteer A's note — fewer bee visits meant fewer flowers made seeds" },
      ],
      correctChoiceId: "d",
    ...
    },
    {
      ...
      correctChoiceId: "a",
      evidenceLogEntry: "Fewer seeds could mean less food for the sparrow. The change moves along the food chain.",
    },
    {
      id: "cp5",
      order: 5,
      position: { x: 72, y: 55 },
      prompt: "Gate 5: The garden club wants more bees next year. Which change would help?",
      evidence: {
        type: "passage",
        text: "The garden club is deciding between four ideas for next season.",
      },
      choices: [
        { id: "a", text: "Paint the garden fence a brighter color" },
        { id: "b", text: "Plant lots of the same flower, all blooming in one week" },
        { id: "c", text: "Put up bright string lights around the garden at night" },
        { id: "d", text: "Plant flowers that bloom at different times, so bees always have food" },
      ],
      correctChoiceId: "d",
      evidenceLogEntry: "Flowers that bloom all season give bees food all season. That brings more bees.",
    },
    {
      id: "cp6",
      order: 6,
      position: { x: 88, y: 35 },
      prompt: "Gate 6: A neighbor says, \"Bees don't matter in this garden.\" Which clue tests that claim?",
      ...
      evidence: {
        ...
        text: "The club puts two marigold plants in mesh cages. Cage A keeps bees out. Cage B lets bees in. After three weeks, Cage A has zero seed pods. Cage B has several.",
      },
      choices: [
        { id: "a", text: "Cage A let more sunlight in" },
        { id: "b", text: "Cage A (no bees) had zero seed pods. Cage B (bees) had several." },
        { id: "c", text: "Both marigolds were the same species" },
        { id: "d", text: "Three weeks passed during the test" },
      ],
      correctChoiceId: "b",
      evidenceLogEntry: "Without bees, the marigolds made zero seed pods. Bees really do matter here.",
    },
  ],

  finalResponsePrompt:
    "Use your Evidence Log to tell the story of this garden's food chain. A neighbor says bees do not matter. Use the cage test to show why that is wrong. Your answer should: (1) tell how energy moves from the Sun to the flowers to the bees, (2) tell what happens to the sparrow when fewer bees visit, and (3) use the cage test to show that bees matter.",
  ...
  selfCheckQuestions: [
    ...
    "I used the cage test to answer the \"bees don't matter\" claim.",
    "I used two pieces of evidence from my Evidence Log.",
