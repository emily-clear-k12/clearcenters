// SERVER ONLY.
// Assembly Deck — ELA.3.12B-AD.

export const SERVER_CASE = {
  standard: "ELA.3.12B-AD",
  title: "The Lunchroom Recycling Report",
  rounds: {
    r1: {
      key: { topic: ["r1p1"], details: ["r1p2", "r1p3"], conclusion: ["r1p4"] },
      decoys: {
        r1p5: "This is a personal story. It is funny, but it does not tell readers how the bins work.",
        r1p6: "The notes say the opposite — every bin has a picture label.",
      },
      decoyReason: { r1p5: "story", r1p6: "contradicts" },
      misplacementNotes: {
        r1p1: "This tells readers what the paragraph is about, so it goes first.",
        r1p2: "This is a true detail from the notes.",
        r1p3: "This is a true detail from the notes.",
        r1p4: "This wraps up the paragraph, so it goes last.",
      },
    },
    r2: {
      key: { topic: ["r2p1"], details: ["r2p2", "r2p3"], conclusion: ["r2p4"] },
      decoys: {
        r2p5: "This is an opinion. Informational writing gives readers facts and lets them decide.",
        r2p6: "The garden is real, but this article is about the recycling bins.",
      },
      decoyReason: { r2p5: "opinion", r2p6: "offtopic" },
      misplacementNotes: {
        r2p1: "This tells readers what the paragraph is about, so it goes first.",
        r2p2: "This is a true detail from the notes.",
        r2p3: "This is a true detail from the notes.",
        r2p4: "This ends the article, so it goes last.",
      },
    },
    r3: {
      key: { topic: ["r3p1"], details: ["r3p2", "r3p3"], conclusion: ["r3p4"] },
      decoys: {
        r3p5: "This is an opinion about punishment, and the article is telling readers how to help, not who to punish.",
        r3p6: "The notes say the helpers are there at lunch, not only on Fridays.",
      },
      decoyReason: { r3p5: "opinion", r3p6: "contradicts" },
      misplacementNotes: {
        r3p1: "This tells readers what the paragraph is about, so it goes first.",
        r3p2: "This is something a reader can actually do — a detail.",
        r3p3: "This is something a reader can actually do — a detail.",
        r3p4: "This ends the article, so it goes last.",
      },
    },
  },
  assemblyKey: { first: "r1", second: "r2", third: "r3" },
  assemblyNote: "An informational article introduces the thing, reports what it has done, and only then asks the reader to do something.",
  decoyProtest: {
    r1p5: "But the banana peel story is GOOD. Everybody laughed. Ask anyone.",
    r1p6: "No labels! ...There are labels. I can see them from here.",
    r2p5: "Most important thing EVER. I stand by it. Loudly. With no evidence.",
    r2p6: "The garden is lovely though. Doesn't the garden get a paragraph?",
    r3p5: "Lose their recess! Sorting is serious! ...That's me being bossy, not informative.",
    r3p6: "Only Fridays! ...The notes say lunch. Every lunch. I made Fridays up.",
  },
  requester: { name: "Ms. Alvarez", emoji: "📰", replies: {
    great: "This is going in the newsletter exactly as you built it. You told readers what the bins are, what changed, and how to help — and you kept the opinions and the funny stories out. That is informational writing.",
    good: "Nice work — this is printable. A sentence or two crept in that was more your opinion than information, but readers will learn what they need to know.",
    rough: "Good start. Right now some of it is telling readers facts and some of it is telling them what you think. Let's read it together and sort out which is which.",
  } },
  trap: { roundId: "r2", position: 2, text: "Everyone in the school loves the new bins.", why: "Nobody counted. \"Everyone loves it\" sounds like information but it is an opinion wearing a fact's coat." },
  mustInclude: [
    "Names at least two sentences that were left out.",
    "Says what kind of sentence each one is — an opinion, a personal story, something that contradicts the notes, or something off topic.",
    "Says that informational writing gives readers true information about the topic.",
  ],
  modelAnswer: "I left out the story about my brother because it is a story about me, not information about the bins. I left out the sentence saying recycling is the most important thing because that is an opinion. An informational article is supposed to tell readers facts about the topic.",
  aiContext: "Grade 3 ELAR. Target: telling informational writing apart from opinion, narrative, and off-topic material. Accept short, simple answers. Naming two sentences with one clear reason each, plus a sentence about what informational writing does, is a 2.",
};
