...
export function gradeExhibit(standard, body) {
  ...
  const misses = portrait ? portrait.misses : split ? split.misses : wall.filter((id, index) => !id || card(exhibit, id)?.job !== exhibit.spots[index].id).length;
  const myth = portrait ? portrait.myth : split ? split.myth : wall.includes(exhibit.mythId);
  const leftover = portrait ? portrait.leftover : split ? split.leftover : (body.bin ? card(exhibit, body.bin) : null);
  const binOk = portrait ? portrait.binOk : !!leftover && leftover.job === "none" && body.reason === leftover.reason;
  let wallLevel = 2;
