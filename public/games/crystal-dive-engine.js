(function () {
  ...
  function finish() {
    ...
    show('<small>EXPEDITION COMPLETE · LAYER ' + layer + '</small><h1>Your Crystal Dive results</h1><div class="recap-grid"><div class="recap-card"><h2>Questions</h2><strong id="resultPercent">' + percent + '% correct</strong><div id="resultCorrect">' + correct + ' of ' + answers.length + ' correct</div></div><div class="recap-card"><h2>Crystals collected</h2><strong id="resultCollected">✦ ' + crystals + '</strong><div>Your spin decides the award.</div></div><div class="recap-card" style="grid-column:1/-1"><h2 id="practiceTitle">' + (correct === answers.length && answers.length ? 'No misses! Keep these facts sharp' : 'Three facts to practice') + '</h2><div id="practiceFacts">' + practice + '</div></div></div><div class="spinner-area"><div class="spinner-wrap"><div class="spinner-pointer"></div><div id="wheel" class="spinner-wheel" role="img" aria-label="Crystal prize wheel with six possible awards">' + wheel + '</div></div><p id="spinStatus" class="spinner-message" role="status">' + (spinsLeft === 2 ? 'Perfect run! You earned two spins.' : 'Spin to reveal your crystal award.') + '</p><button class="primary" id="spin">Spin the crystal wheel</button><div class="spinner-prize">Crystals awarded: <strong id="banked">0</strong></div></div><button class="primary" id="again" disabled>Finish your spin first</button>');
