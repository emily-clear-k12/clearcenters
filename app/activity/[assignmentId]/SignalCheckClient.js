...
//
// Sept 16, 2026 — Signal Check is one format again. Two companion formats
// ("Weigh-In" — pick between two sides; "Thread" — flag comments on a
// claim) briefly ran alongside this one, selected per case via a
// `caseShape` field. Both are gone: Thread never asked the student to
// judge the claim at all, and neither graded the evidence the student
// picked — it only counted that two things were picked, so a student who
// reasoned carefully scored the same as one who tapped the same button
// repeatedly. The Weigh-In cases are parked in
// lib/cases/_archive/signal-check-weigh-in/ with a note on what bringing
// them back would take; the Thread cases were removed. The phase list,
// evidence shape and submit payload below are all single-format again.
...
function normalizeEvidence(publicCase) {
...
export default function SignalCheckClient({ assignmentId, studentId, caseStandard, publicCase, existingSubmission, alreadySubmitted, revisionRequested, revisionFeedback, samSkin, samNickname }) {
  ...
  const activeSortBins = publicCase.sortBins || [];
