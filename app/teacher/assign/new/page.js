...
import { codesFor, topicForCase, coverLine, frUnitKey, frTopicName } from "../../../../lib/standardCodes";
...
function displayCode(code){return String(code||'').replace(/^TEKS\s+/i,'').replace(/^(MA|ELA|ELAR|SS|SCI)\./i,'');}
  ...
  const visibleGroups = standardGroups.filter((group) => !searchQ || group.code.toLowerCase().includes(searchQ) || String(group.title || "").toLowerCase().includes(searchQ));
  const standardFamilies = visibleGroups.reduce((list, group) => {
  ...
  return <BridgePage teacherEmail={teacherEmail}><PageHeading title="Find your next activity" subtitle="Choose a topic. Find the right experience. Make it yours."><ClassTabs classes={classes} value={assignClassId} onChange={id=>{setAssignClassId(id);setFollowClass(true);setLane('standard');setTypeFilter('all');resetBrowse();setCaseSearch('')}}/><div className="cc-class-context">{targetClass?.name || 'Choose a class'} · {roster.length} students</div></PageHeading>
    ...
    {inRush && rushMode && !rushStandard && <section className="cc-panel"><button className="cc-text-button" onClick={()=>{setRushMode(null);setSelectedCase(null);setCaseSearch('');}}>← Run, Dive, Crew</button><h2>{rushTitle}</h2><p className="cc-muted">{browseSubject} · Grade {browseGrade}. Choose a topic.</p><div className="cc-rush-board" style={subjectStyle(browseSubject)}>{[["Vocab","Vocab"],["Topic questions","Topic questions"]].map(([label,kind])=>{const items=rushTopics.filter((topic)=>topic.kind===kind);return <section key={kind}><h3>{label}</h3><div className="cc-rush-topics">{items.map((topic)=><button key={topic.key} type="button" className="cc-rush-topic" onClick={()=>{setRushStandard(topic.key);setSelectedCase(null);setCaseSearch('');setLimit(12)}}>{topic.image&&<img src={topic.image} alt="" onError={(event)=>{const img=event.currentTarget;const list=topic.images||[];const step=Number(img.dataset.step||0)+1;if(step>=list.length){img.style.visibility="hidden";return;}img.dataset.step=String(step);img.src=list[step];}}/>}<b>{displayCode(topic.code)}</b><strong>{topic.name}</strong><small>{topic.kind}</small></button>)}{!items.length&&<p className="cc-muted">None for this grade yet.</p>}</div></section>})}</div>{!rushTopics.length&&<Empty>{casesLoading?'Loading games…':`No ${rushTitle} sets for this grade and subject yet.`}</Empty>}</section>}
