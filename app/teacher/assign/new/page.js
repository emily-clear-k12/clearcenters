...
function displayCode(code){return String(code||'').replace(/^TEKS\s+/i,'').replace(/^(MA|ELA|ELAR|SS|SCI)\./i,'');}
  ...
  const rushTopics = !inRush || !rushMode ? [] : Object.values(cases.map(normalizeCaseRow).reduce((groups, c) => {
    ...
    if (!groups[key]) groups[key] = { key, code: codes.join(" & ") || displayCode(key), name, kind: isVocabSet(c.standard) ? "Vocab" : "Topic questions", image: caseImageCandidates(unit || c.standard, c.engine)[0], count: 0, seen: new Set() };
