...
// doc §10.3/§10.5). The art has 2 usable zones: a circular gauge display
// (upper-left) and a horizontal slider slot (center). Adjust these if the
// art ever changes.
//
// Sept 4 fix: there used to be a 3rd zone here, readoutPanel (upper-right),
// for the digital "screen" readout. It's been removed — that readout is
// now a normal-flow panel below the console art instead of a 3rd
// percentage-positioned overlay zone, because it could grow taller than
// its box and visually cover the gauge dial (see the comment above the
// Console() image wrapper for the full explanation). Don't re-add a
// readoutPanel hotspot without fixing that underlying issue first.
...
export default function SimulationLabClient({
  ...
  // { submittedText } } for hyp/cp1/cp2 — a generic shape shared by the
  // mc/dropdown/multiSelect/fillBlank CheckpointCard below, mirroring what
  // the submit route expects.
    ...
    try {
      const checkpointResults = publicCase.checkpoints.map((cp) => {
        ...
        if (cp.type === "fillBlank") {
          return { id: cp.id, type: cp.type, submittedText: ans.submittedText || "" };
        }
  ...
  // fillBlank, reused for the pre-trial hypothesis check, Checkpoint 1,
  // and Checkpoint 2 (design doc §10.2 point 3 / §10.5 / §5.4 for
  // fillBlank, added when the grade-5 rigor tier's first authored case
  // needed it — the schema/route already supported fillBlank scoring, but
  // this card had never actually rendered an input for it). ------------
  function CheckpointCard({ cp, nextLabStep, samCategory = "checkpointAnswered" }) {
    ...
    const isFillBlank = cp.type === "fillBlank";
    ...
    function handleSubmit() {
      ...
      if (isMulti) {
        ...
        setCheckpointAnswers((prev) => ({ ...prev, [cp.id]: { submittedText: pendingText.trim() } }));
      } else {
      ...
      <div style={{ background: COLORS.panel, border: `1px solid ${COLORS.amber}55`, borderRadius: 16, padding: 18, marginBottom: 18 }}>
        ...
        {isFillBlank && (
          <input
            ...
            style={{
              width: "100%", borderRadius: 10, padding: 10, fontSize: 13.5,
              border: `1px solid ${COLORS.steelLine}`, background: "#EEF3FF", color: "#1A2432",
  ...
  function Console() {
    ...
    return (
      <div style={{ background: COLORS.panel, border: `1px solid ${COLORS.steelLine}`, borderRadius: 16, padding: 18, marginBottom: 18 }}>
        ...
        {hasBgImage ? (
          // Real console art (design doc §10.3) — the dial and prediction
          // slider are positioned directly on the image's own gauge circle
          // / slider slot via CONSOLE_HOTSPOTS, tuned against this exact
          // image. The digital "screen" readout used to be a third
          // absolutely-positioned zone here too (readoutPanel) — Sept 4
          // fix: pulled it out into normal document flow below the image
          // instead (see the panel right after this wrapper closes),
          // because a percentage-height box positioned over an
          // auto-height image container doesn't reliably stay pinned to
          // its intended size once it needs to show more lines of text
          // (hypothesis + angle + predict + actual + gap all at once) —
          // it was growing tall enough to visually cover the gauge dial
          // below it. Rendering it in normal flow means it can never
          // overlap anything else on the console art, regardless of how
          // much text a given case's readout needs to show.
          <>
          <div style={{ position: "relative", marginBottom: 16 }}>
            ...
            {/* Prediction slider — sits in the art's slot track now that
                the dial above owns the angle control. Sept 4 fix: added
                +/- stepper buttons flanking the slider (Emily's live-test
                feedback that dragging to an exact value is hard,
                especially on wide ranges) and enlarged the slider's own
                click/touch height while keeping the visual track thin. */}
            <div
              style={{
                position: "absolute",
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <button
                  type="button"
                  className="sl-step-btn"
                  disabled={atMax || currentPrediction <= outcome.displayMin}
                  onClick={() => { setCurrentPrediction(Math.max(outcome.displayMin, currentPrediction - 1)); setPredictionTouched(true); }}
                  aria-label="Decrease prediction"
                >
                  −
                </button>
                <input
                  ...
                  style={{ flex: 1, "--thumb-color": COLORS.cyan }}
                />
                <button
                  type="button"
                  className="sl-step-btn"
                  disabled={atMax || currentPrediction >= outcome.displayMax}
                  onClick={() => { setCurrentPrediction(Math.min(outcome.displayMax, currentPrediction + 1)); setPredictionTouched(true); }}
                  aria-label="Increase prediction"
                >
                  +
                </button>
              </div>
              <div style={{ textAlign: "center", fontSize: "clamp(9px, 1.3vw, 13px)", color: readoutColor, fontWeight: 800, marginTop: 4 }}>
          ...
          {/* Digital "screen" readout — Emily's ask for something in the
              ...
              carrying the student's own hypothesis so the console visibly
              connects back to the choice they made on the previous step.
              Sept 4 fix: now a normal-flow panel below the console art
              (was absolutely positioned over it — see the comment above
              the image wrapper) so it sizes to its own content instead of
              fighting a fragile percentage-of-image-height box. */}
          <div
            style={{
              background: "#0B1017", borderRadius: 10, border: `1px solid ${COLORS.cyan}66`,
              boxShadow: "inset 0 0 12px rgba(62,214,200,.25)",
              display: "flex", flexDirection: "column", gap: 6,
              padding: "10px 14px", marginBottom: 16, fontFamily: "'Courier New', monospace",
            }}
          >
            {hypChoice && (
            ...
            <div style={{ display: "flex", flexWrap: "wrap", columnGap: 20, rowGap: 4 }}>
              <div style={{ fontSize: 13, color: COLORS.cyan, fontWeight: 700, textShadow: `0 0 6px ${COLORS.cyan}88` }}>
              ...
              {lastRun && (
                <>
                  <div style={{ fontSize: 13, color: COLORS.amber, fontWeight: 700, textShadow: `0 0 6px ${COLORS.amber}88` }}>
  ...
  return (
    <div style={backgroundStyle}>
      <style>{`
        ...
        .sl-slider { -webkit-appearance: none; appearance: none; width: 100%; height: 30px; background: transparent; outline: none; cursor: pointer; }
        .sl-slider:disabled { opacity: .5; cursor: default; }
        .sl-slider::-webkit-slider-runnable-track { height: 10px; border-radius: 999px; background: rgba(255,255,255,.35); }
        ...
        /* Stepper buttons beside prediction sliders (Sept 4 fix) — precise
           single-unit nudging for kids who find dragging imprecise,
           especially on wide ranges (e.g. 0-50). */
        .sl-step-btn {
          flex: 0 0 auto; width: 34px; height: 34px; border-radius: 50%;
          border: 1px solid rgba(255,255,255,.25); background: rgba(255,255,255,.08);
          color: #EAF0F6; font-size: 18px; font-weight: 700; line-height: 1;
          display: flex; align-items: center; justify-content: center;
          cursor: pointer; transition: background 150ms ease, transform 150ms ease;
          font-family: 'Inter', sans-serif;
        }
        .sl-step-btn:hover:not(:disabled) { background: rgba(255,255,255,.18); transform: translateY(-1px); }
        .sl-step-btn:disabled { opacity: .35; cursor: default; }
      `}</style>
